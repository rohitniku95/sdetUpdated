...
public class CreateNewContact1 {
	public static void main(String[] args) throws IOException, InterruptedException {
		//to generate random number to avoid duplicacy
		Random ran=new Random();
		int ranDomNum = ran.nextInt(1000);
		//create physical address to read data from property file
		FileInputStream fis=new FileInputStream("./data/ComData.properties");
		Properties Pobj=new Properties();
		Pobj.load(fis);
		//to read the data from property file we can use getProperty
		...
		FileInputStream fis_e=new FileInputStream("./data/testScriptData.xlsx");
		 ...
		 Sheet sh = wb.getSheet("ContactModule");
		 Row row=sh.getRow(1);	
		 ...
		 if(text.contains(ContactName)) {
			 System.out.println("Contact is created sucessfully");
		 }else {
